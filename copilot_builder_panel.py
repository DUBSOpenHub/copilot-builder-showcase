#!/usr/bin/env python3
"""
Copilot Builder Panel — single-file MVP
...
def import_url_submissions(bundle_path: Path, urls: List[str],
    ...
    for url in urls:
        ...
        if sub_path.exists():
            continue
        summary_bits = [f"Repository submitted for Copilot Builder Panel judging: {url}"]
...
def import_url_submissions(bundle_path: Path, urls: List[str],
    ...
    for url in urls:
        ...
        submission = {
            ...
            "builder_name": builder_name,
            "project_name": owner_repo,
            "description": f"Repository submitted for Copilot Builder Panel judging: {url}",
            "repo_url": url,
            "artifacts": [],
            "submitted_at": _now(clock),
            "file_size_bytes": 0,
...
def build_feedback_cards(
    ...
    for scored in scored_submissions:
        ...
        prompt = (
            f"You are the Copilot Builder Panel. Write an encouraging feedback card for this builder.\n\n"
            f"Project: {sub.get('project_name', 'Unknown')}\n"
            f"Builder: {sub.get('builder_name', 'Unknown')}\n"
            f"Description: {sub.get('description', '')}\n\n"
...
def cmd_init(args: argparse.Namespace, _gateway: Optional[Any] = None,
    ...
    if not getattr(args, "quiet", False):
        _magic_banner("Copilot Builder Panel", "The panel is assembling. The build is about to shine.")
...
def _write_awards_markdown(bundle_path: Path, awards_card: Dict) -> None:
    lines = ["# Copilot Builder Panel Awards", ""]
    ...
    lines += ["> Generated by Copilot Builder Panel. Human approval required before external publishing.", ""]
...
def _share_card(awards_card: Dict, run_id: str) -> None:
    ...
    print(_paint("│" + _truncate(f"The Copilot Builder Panel · {run_id}", width - 4).center(width) + "│", "cyan"))
...
def cmd_workshop(args: argparse.Namespace, _gateway: Optional[Any] = None,
    ...
    _magic_banner("Copilot Builder Panel Live", "Three judges. One envelope. Tonight's room.")
    ...
    elif not _confirm("Start the Copilot Builder Panel judging?"):
        return 0
...
def cmd_judge(args: argparse.Namespace, _gateway: Optional[Any] = None,
    ...
    if showtime:
        _magic_banner("The Copilot Builder Panel", "The Spark · The Builder · The Champion")
        _sideline("Premium judges are taking their seats. No teardowns. Only spotlights.", "🏟️", "magenta")
    else:
        _magic_banner("Copilot Builder Panel", "Premium judges are taking their seats.")
        _sideline("The builder panel is warming up: fresh models, sealed scores, no teardown.", "🏟️", "magenta")
    _showtime_pause(args)
    if showtime:
...
def cmd_present(args: argparse.Namespace, _gateway: Optional[Any] = None,
    ...
    _magic_banner("Copilot Builder Panel", f"Run: {run_id} · Mode: {manifest.get('mode', 'workshop').upper()}")
...
def cmd_award(args: argparse.Namespace, _gateway: Optional[Any] = None,
    ...
    winner_md = (
        ...
        "> Generated by Copilot Builder Panel. Human approval required before external publishing.\n"
    )
...
def cmd_recap(args: argparse.Namespace, _gateway: Optional[Any] = None,
    ...
    lines = [
        f"# Copilot Builder Panel Recap — {run_id}",
        "",
...
def cmd_tui(args: argparse.Namespace, _gateway: Optional[Any] = None,
    ...
    if run_id:
        _magic_banner("Copilot Builder Panel Live Board", "Artifact-powered spotlight mode")
        return cmd_present(args, _gateway, clock)

    _magic_banner("Copilot Builder Panel Live Board", "Choose a sealed run to present")
...
def cmd_list(args: argparse.Namespace, _gateway: Optional[Any] = None,
    ...
    _magic_banner("Copilot Builder Panel Runs", "Every run is replayable. Every bundle is proof.")
...
def cmd_replay(args: argparse.Namespace, _gateway: Optional[Any] = None,
    ...
    _magic_banner("Copilot Builder Panel Replay", f"Run: {manifest.get('run_id', bundle_path.name)}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Copilot Builder Panel — CLI judging for hackathons and builder programs.",
    )
