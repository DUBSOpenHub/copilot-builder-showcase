#!/usr/bin/env python3
"""Refresh ``demo_projects.json``, the bundled practice-showcase project pool.
...
def source_repos(argv: List[str]) -> List[str]:
    ...
    return [p["repo"]["name_with_owner"] for p in current["projects"]]
...
def embed_in_page(projects: List[Dict[str, Any]]) -> None:
    ...
    GitHub Pages cannot read ``demo_projects.json`` at runtime for an offline
    practice run, so the page carries its own copy. Generating both from here
    keeps the two from drifting; ``tests/test_demo_pool_parity.py`` fails if
    they ever do.
    """
    rows = []
...
def main(argv: List[str]) -> int:
    ...
    OUT.write_text(
        json.dumps(
            {"rotation_size": ROTATION_SIZE, "projects": projects},
            indent=1,
            ensure_ascii=False,
        )
        + "\n",
